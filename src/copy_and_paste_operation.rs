...
pub(crate) fn execute_copy_operation(files: Signal<Files>, clicked_directory_id: &Mutex<usize>) {
...
pub(crate) fn execute_paste_operation(mut files: Signal<Files>, previous_operation_done: &Mutex<String>) {
...
fn paste_operation(selected_current_stack: String, copied_file_or_dir_name_joined: String, files: Signal<Files>, previous_operation_done: &Mutex<String>) {
...
fn paste_file(selected_current_stack: String, copied_file_or_dir_name_joined: String, files: Signal<Files>, previous_operation_done: &Mutex<String>) {
    ...
    end_paste_file_operation(
        selected_current_stack.clone(),
        file_name,
        copied_file_or_dir_name_joined.clone(),
        previous_operation_done
    );
}
...
fn get_restructured_file_name(file_name: String) -> String {
    *COPY_INCREMENTAL_ID.lock().unwrap() += 1;
    ...
    format!(
        "{splitted_file_name} Copy {copy_index}{splitted_file_extension}",
        splitted_file_name = splitted_file_name,
        copy_index = COPY_INCREMENTAL_ID.lock().unwrap(),
        splitted_file_extension = splitted_file_extension
    )
}

fn end_paste_file_operation(
    mut selected_current_stack: String,
    file_name: String,
    copied_file_or_dir_name_joined: String,
...
fn paste_dir(mut selected_current_stack: String, copied_file_or_dir_name_joined: String, files: Signal<Files>, previous_operation_done: &Mutex<String>) {
    let copy_options: CopyOptions = CopyOptions::new();

    if conflict_process::check_file_or_dir_conflict(
        copied_file_or_dir_name_joined.split("\\").last().unwrap().to_string(),
        selected_current_stack.clone(),
        files
    ) && (window_helper::get_file_type_formatted(copied_file_or_dir_name_joined.clone()) == REGULAR_FILE.to_string()).not() {
        paste_dir_with_conflict(
            selected_current_stack.clone(),
            copied_file_or_dir_name_joined.clone(),
            previous_operation_done,
            &copy_options
